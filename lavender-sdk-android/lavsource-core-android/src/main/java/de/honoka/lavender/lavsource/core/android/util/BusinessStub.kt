...
abstract class WebBusinessStub : BusinessStub() {
    ...
    protected fun requestForAny(request: HttpRequest): Any? {
        ...
        val resObj = response.toJsonWrapper().toBean<ApiResponse<Any?>>()
        if(!resObj.success!!) {
            resObj.exceptionDetails.throwThis()
        }
        return resObj.data
    }

    protected inline fun <reified T> request(request: HttpRequest): T? =
        requestForAny(request)?.tryCastOrNull(typeOf<T>())

    protected inline fun <reified T> getOrNull(path: String, block: HttpRequest.() -> Unit = {}): T? =
        request(HttpUtil.createGet(url(path)).apply(block))

    protected inline fun <reified T> get(path: String, block: HttpRequest.() -> Unit = {}): T =
        getOrNull(path, block)!!

    protected inline fun <reified T> postOrNull(path: String, block: HttpRequest.() -> Unit = {}): T? =
        request(HttpUtil.createPost(url(path)).apply(block))

    protected inline fun <reified T> post(path: String, block: HttpRequest.() -> Unit = {}): T =
        postOrNull(path, block)!!
}

abstract class AndroidBusinessStub : BusinessStub() {

    protected fun callProvider(businessFunction: KFunction<*>, args: Iterable<Any?>? = null): Any? =
        typedCallProvider(businessFunction, args)

    protected fun <T> typedCallProvider(businessFunction: KFunction<*>, args: Iterable<Any?>? = null): T =
